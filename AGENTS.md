...
Rakuraku Music Station — a community radio web app: users request songs, an embedded audio engine plays them into an MP3 stream, and a Vue SPA shows now-playing, lyrics, queue, and admin panels. The project is Chinese-language (code comments and UI strings are often zh-CN).
...
**State path**: the engine publishes `PlaybackState` every 500 ms; `services/playback_broadcast.rs` polls it and broadcasts over WS. WS message types: `playback_state`, `queue_update`, `notice`, `ping`, `listeners_update`. **Lyrics contract**: full `lyrics_lines` is sent only once per song change; the 500 ms updates carry `lyrics_line` + `lyrics_lines: null`. The frontend (`api/websocket/messages.ts`) only overwrites `store.lyricsLines` when `lyrics_lines` is non-null.

**Frontend**: one global reactive store (`src/store.ts`, NOT Pinia) fed by REST (`api/client.ts` fetch wrapper) and the WS client (`api/websocket/`). When WS is closed, `fallbackPolling.ts` polls `/api/now-playing` every 2 s and queue every 5 s; `interpolation.ts` smooths `displayPositionMs` between updates.
...
- `radio-backend/frontend/` — Vue 3 + Vuetify 3 + Tailwind v4 SPA: `src/views/`, `src/components/` (`lt/`, `admin/`), `src/api/` (+ `websocket/`), `src/app/`, `src/composables/`, `store.ts`, `router.ts`, `style.css`, `types.ts`
...
- Frontend type-check + build: `cd radio-backend/frontend && npm run build` (= `vue-tsc -b && vite build`, writes `../static/`)
...
**Frontend (Vue 3 / TS)**:
- State: one global reactive store (`src/store.ts`, typed `interface Store`); api modules mutate the store directly and `toast(message, level)` on failure.
- REST: `apiFetch` wrapper (`fetch`, `credentials: 'same-origin'`, base = `window.location.origin` + BASE_URL); parse `ApiResponse<T> {success, data?, error?}`.
- WS: `connectWebSocket()` with exponential backoff (20 attempts, 3 s × 1.5ⁿ capped 30 s); dispatch in `websocket/messages.ts`; preserve the lyrics cache per the contract above.
- Naming: PascalCase `.vue` components, `useX` composables, kebab-case api modules, camelCase helpers.
- CSS overflow convention — apply **all three** when text may overflow: `clamp()` font-size, wrap (`overflow-wrap`/`word-break`) or truncate (`text-overflow: ellipsis`), and layout adjustments (`min-width: 0`, flex/grid). Utilities in `style.css`: `.lt-text-wrap`, `.lt-text-truncate`, `.lt-text-responsive`; theme tokens `--am-*` / `--lt-*` toggled via `data-theme` / `data-layout`.
...
- `radio-backend/frontend/src/store.ts`, `api/client.ts`, `api/websocket/{connection,messages,fallbackPolling,interpolation}.ts`, `style.css`
...
- Toolchain: Rust edition 2021, tokio, axum 0.7, sqlx (sqlite), tower-http; Node with Vite 6, vue-tsc, TypeScript ~5.7 strict.
...
- Verification is type-check/build plus manual smoke test: `npm run build` (`vue-tsc -b`) for the frontend, `cargo build` for the backend, then exercise the changed path end to end (e.g. drive `/ws` and `/stream` in a browser).
