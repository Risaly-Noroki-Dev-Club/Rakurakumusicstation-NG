...
# Debug build — output to root (binary is gitignored; rebuild after editing)
g++ radioserver.cpp metadata.cpp -o radioserver \
    -std=c++17 -g -O0 -lpthread -lssl -lcrypto -I.

# Manual release build (equivalent to build_release.sh without template embedding)
# First generate embedded_templates.hpp:
python3 -c "
import os
lines=['#pragma once','#include <string>','#include <unordered_map>','namespace EmbeddedTemplates {','static const std::unordered_map<std::string, std::string> templates = {',]
for f in ['index.html','panel.html','login.html','manifest.json','sw.js']:
    if os.path.exists(f):
        with open(f,encoding='utf-8') as fh: c=fh.read()
        lines.append(f'  {{\"{f}\", R\"RKTML('); lines.append(c); lines.append(')RKTML\"},')
lines+=['};','}']
with open('embedded_templates.hpp','w',encoding='utf-8') as o: o.write('\n'.join(lines)+'\n')
"
g++ radioserver.cpp metadata.cpp -o dist/radioserver \
    -std=c++17 -O3 -flto -march=native -lpthread -lssl -lcrypto -I. -w
```

> **`crow_all.h` and `embedded_templates.hpp` are build artifacts and gitignored.**
> - `crow_all.h` — download once before building (see the `wget` command above). Re-download to update.
> - `embedded_templates.hpp` — generated by `build_release.sh` (or manually with the Python snippet above). Regenerate when templates change.
...
├── crow_all.h               # Crow header-only web framework (download before build; gitignored)
├── embedded_templates.hpp   # Auto-generated during build — embedded HTML/JS/SW templates (gitignored)
│
├── build_release.sh         # One-shot release build script
...
├── README.md                # This file
│
├── dist/                    # Runtime deployment directory (preserved across rebuilds; gitignored)
...
│   ├── settings.json        # Runtime configuration (gitignored — contains credentials)
│   ├── playlist_order.json  # Persisted playlist ordering (preserved across rebuilds)
│   ├── music_dl.py          # Copied from root during build
│   ├── requirements.txt     # Copied from root during build
│   ├── server.log           # Runtime log (gitignored)
│   ├── .server.pid          # PID file for stop.sh (gitignored)
...
# 调试构建 — 输出到根目录（二进制文件已 gitignore；修改源码后重新构建）
g++ radioserver.cpp metadata.cpp -o radioserver \
    -std=c++17 -g -O0 -lpthread -lssl -lcrypto -I.

# 手动发布构建（等价于 build_release.sh，不含模板内嵌）
# 先生成 embedded_templates.hpp：
python3 -c "
import os
lines=['#pragma once','#include <string>','#include <unordered_map>','namespace EmbeddedTemplates {','static const std::unordered_map<std::string, std::string> templates = {',]
for f in ['index.html','panel.html','login.html','manifest.json','sw.js']:
...
> **`crow_all.h` 和 `embedded_templates.hpp` 是构建产物，已加入 gitignore。**
> - `crow_all.h` — 构建前手动下载一次（见上方 `wget` 命令）。需要更新时删除重新下载。
> - `embedded_templates.hpp` — 由 `build_release.sh`（或上方 Python 代码段）生成。模板变更后需重新生成。
...
├── crow_all.h               # Crow 纯头文件 Web 框架（构建前下载；已 gitignore）
├── embedded_templates.hpp   # 构建时自动生成 — 内嵌 HTML/JS/SW 模板（已 gitignore）
│
├── build_release.sh         # 一键发布构建脚本
...
├── README.md                # 本文件
│
├── dist/                    # 运行时部署目录（重编译时保留；已 gitignore）
│   ├── radioserver          # 编译后的服务器二进制文件
│   ├── start.sh             # 启动脚本（设置 zh_CN.UTF-8 区域、后台运行服务器）
│   ├── stop.sh              # 优雅关闭（SIGTERM → 10 秒后 SIGKILL）
│   ├── settings.json        # 运行时配置（已 gitignore — 包含凭据）
│   ├── playlist_order.json  # 持久化播放顺序（重编译时保留）
│   ├── music_dl.py          # 构建时从根目录复制
│   ├── requirements.txt     # 构建时从根目录复制
│   ├── server.log           # 运行时日志（已 gitignore）
│   ├── .server.pid          # stop.sh 使用的 PID 文件（已 gitignore）
│   ├── media/               # 音频文件目录（已 gitignore）
