...
- **One-shot build** — `build_release.sh` installs deps, fetches Crow, compiles, and produces `dist/`
...
> **Re-downloading Crow**: the build script skips the download if `crow_all.h` already exists. Delete it before rebuilding if you want to pull the latest release.
...
| `separate_stream_port` | `false` | `true` → audio stream on port 2241; `false` → everything on 2240 |

### Port Modes

| Mode | Web / API | Audio Stream |
|------|-----------|--------------|
| Single-port (default) | `:2240` | `:2240/stream` |
| Dual-port (`separate_stream_port: true`) | `:2240` | `:2241` |
...
├── authmiddleware.hpp     # Auth middleware (defined but not wired in)
...
| Port already in use | `ss -ltnp \| grep -E ':2240\|:2241'`; edit `Config::WEB_PORT` / `Config::STREAM_PORT` in `radioserver.cpp` |
...
- **一键构建** — `build_release.sh` 自动安装依赖、下载 Crow 并生成 `dist/`
...
> **更新 Crow**：构建脚本发现 `crow_all.h` 已存在时会跳过下载。若需拉取最新版，删除该文件后重新运行构建脚本即可。
...
| `separate_stream_port` | `false` | `true` → 音频流走 2241 端口；`false` → 全部在 2240 |

### 端口模式

| 模式 | Web / API | 音频流 |
|------|-----------|--------|
| 单端口（默认） | `:2240` | `:2240/stream` |
| 双端口（`separate_stream_port: true`） | `:2240` | `:2241` |
...
├── authmiddleware.hpp     # 认证中间件（已定义，未接入 Crow）
...
| 端口被占用 | `ss -ltnp \| grep -E ':2240\|:2241'`；修改 `radioserver.cpp` 中的 `Config::WEB_PORT` / `Config::STREAM_PORT` |
