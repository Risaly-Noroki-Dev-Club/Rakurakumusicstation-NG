...
4. **Single-port mode**: All services on port 2240 (`Config::WEB_PORT`). No separate stream port.
...
│
├── radio-backend/           # [In Development] Rust backend (Actix-web + SQLite + WebSocket)
├── audio-engine/            # [In Development] C++ audio engine (CMake-based, Redis client)
├── deploy/                  # Systemd deployment manifests (radio-backend, audio-engine, nginx)
├── paper-radio-plugin/      # [In Development] Plugin placeholder
...
4. **单端口模式**：所有服务在 2240 端口（`Config::WEB_PORT`）。无独立流端口。
...
│
├── radio-backend/           # [开发中] Rust 后端（Actix-web + SQLite + WebSocket）
├── audio-engine/            # [开发中] C++ 音频引擎（基于 CMake，Redis 客户端）
├── deploy/                  # Systemd 部署清单（radio-backend、audio-engine、nginx）
├── paper-radio-plugin/      # [开发中] 插件占位
