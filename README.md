...
| 队列不前进 | C++ 引擎需连接 Redis 以接收指令；或重启服务 |
...
| 服务 | 语言 | 端口 | 职责 |
|------|------|------|------|
| 音频引擎 | C++17 | 2240 | ffmpeg 解码 → 环形缓冲 → TCP 推流；Redis pub/sub |
...
| `GET` | `/api/songs/:id/download` | JWT | **（新）** 下载歌曲文件 |
| `POST` | `/api/songs/upload` | JWT | **（新）** 上传歌曲 |
| `GET` | `/api/queue` | 无 | 队列 |
...
| `GET` `POST` | `/api/ncm` | JWT | **（新）** 个人网易云账号 |
| `POST` | `/api/ncm/test` | JWT | **（新）** 测试网易云登录 |
| `GET` `POST` `DELETE` | `/api/admin/*` | Admin | 管理端点 |
...
| 频道 | 方向 | 负载 |
|------|------|------|
| `playback_state` | C++ → Rust | `PlaybackState` JSON |
| `command` | Rust → C++ | `AudioCommand` JSON |
| `queue_event` | Rust → C++ | `QueueEvent` JSON（预留） |
