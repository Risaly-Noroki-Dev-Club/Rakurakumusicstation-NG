...
> **Admin account**: The first user created through the built-in web setup wizard is automatically an admin. See [Admin Setup](#admin-setup) below.
...
On first startup, the web UI presents a full-screen **Setup Wizard**. Enter a username and password — this user becomes the **station administrator**.

If you need additional admin accounts later, an existing admin can promote users:

1. Go to **管理 / Admin → 用户管理 / Users**
2. Find the user and click **⭐ 提权** to promote to admin
3. Click **⬇ 降权** to revoke admin privileges

Admins can also be promoted manually via SQLite if needed:
...
After promotion, the user must log out and log back in for the **管理 / Admin** tab to appear.

### Admin Sub-tabs

| Tab | Functions |
|-----|-----------|
| **用户管理 / Users** | View all users; ban/unban users; promote/demote user roles; view operation logs |
...
| **Can't log in** | Use the first-run setup wizard to create an admin account. Register a new account through the login modal if you need a regular user account |
...
> **管理员账号**：首次启动时，Web 界面会弹出全屏设置向导，输入用户名密码即可创建管理员账户。详见下方[管理员设置](#管理员设置)。
...
首次启动时，Web 界面会显示全屏**设置向导**。输入用户名和密码，该用户即成为**电台管理员**。

如需添加更多管理员，已有管理员可通过界面操作：

1. 进入 **管理 / Admin → 用户管理 / Users**
2. 找到目标用户，点击 **⭐ 提权** 将其提升为管理员
3. 点击 **⬇ 降权** 撤销其管理员权限

也可以通过 SQLite 手动提升（作为备选）：
...
提升后需退出登录重新登录，**管理 / Admin** 标签才会出现。

#### 管理子标签

| 标签 | 功能 |
|------|------|
| **用户管理 / Users** | 查看所有用户；封禁/解封用户；提权/降权用户角色；查看操作日志 |
...
| **无法登录** | 使用首次启动设置向导创建管理员账户。如需普通用户账户，通过登录弹窗注册即可 |
